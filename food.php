<?php
/*
Plugin Name: Chicago Food Widget Powered By Everyblock 
Plugin URI: http://tirespider.com/
Description: Add a Chicago Food Widget to a post or page.
Version: 2.0.4
Author: Jeff S
...
class ChicagoFoodWidget extends WP_Widget {

	public static $metro = "chicago";
	public static $schema = "food-inspections";
	
	public function ChicagoFoodWidget()
	{
		$widget_ops = array('classname' => 'ChicagoFoodWidget', 'description' => 'Embeds a Chicago Food Inspections Widget' );
		$this->WP_Widget('ChicagoFoodWidget', 'Chicago Food Inspections Widget', $widget_ops);
	}
...
add_action('widgets_init', create_function('', 'return register_widget("ChicagoFoodWidget");') );

function display_chicago_food_widget($atts) {
	$atts = extract(shortcode_atts( array('width' => '300', 'height' => '500'), $atts, 'display_chicago_food_widget'));
	
	$code = "<iframe src=\"" . plugins_url( 'widget.php', __FILE__ ) . "?metro=" . ChicagoFoodWidget::$metro . "&schema=" . ChicagoFoodWidget::$schema . "&width=" . $width . "&height=" . $height . "&url=" . ABSPATH . "\" width=\"". $width . "\" //height=\"" . $height . "\" frameborder='0' scrolling='no'></iframe>";
	 
	return get_html_for_embed_chicago_food(stripslashes($code));
}

function get_html_for_embed_chicago_food($embedCode) {
	return '<div class="widget_block">' . stripslashes($embedCode) .'</div>';  
}

add_shortcode('display_chicago_food_widget', display_chicago_food_widget);
